...
export default function SerieDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=${apiKey}&language=fr-FR&append_to_response=videos`)
      .then((r) => r.json())
      .then((d) => setSerie(d));
  }, [id]);

  if (!serie) return <div style={{ textAlign: 'center', padding: '100px', color: 'var(--text-muted)' }}><div className="loading-spinner"></div><p>Chargement...</p></div>;
      ...
      <div className="content-wrapper" style={{ position: 'relative', zIndex: 10, marginTop: '-180px', paddingBottom: '80px' }}>
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', flex: 1, minWidth: '300px' }}>
            <h1 style={{ fontSize: '3rem', fontWeight: 800, marginBottom: '16px', lineHeight: 1.1 }}>{serie.name}</h1>
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', color: 'var(--text-muted)', fontSize: '1.05rem', fontWeight: 500, flexWrap: 'wrap' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ color: '#fff', fontWeight: 700 }}>{serie.vote_average?.toFixed(1)}</span>/10
              </span>
              <span>•</span>
              <span>{serie.first_air_date?.split('-')[0]}</span>
              <span>•</span>
              <span>{serie.number_of_seasons} Saison(s)</span>
              <span>•</span>
              <span>{serie.number_of_episodes} Épisode(s)</span>
            </div>
            ...
            <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <Link to={`/serie/${id}/saisons`} className="menu-btn" style={{ border: '1px solid var(--border-glass)', display: 'flex', alignItems: 'center', gap: '8px', padding: '14px 24px', textDecoration: 'none' }}>
